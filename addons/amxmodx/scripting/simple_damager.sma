...
new const DEFAULT_ATTACKER_FLAGS[] = "qrst";
...
// Per-player toggle: VIP игроки могут скрыть свой дамаг-хад командой /nodmg
new bool:g_bHideDamage[MAX_PLAYERS + 1];
...
public plugin_init()
{
	...
	// VIP can toggle damage display via /nodmg
	register_clcmd("say /nodmg",      "clcmd_toggle_damage");
	register_clcmd("say_team /nodmg", "clcmd_toggle_damage");
}
...
public clcmd_toggle_damage(id)
{
	// Only VIP-flagged players (flags qrst) can use this command
	if (!(get_user_flags(id) & read_flags("qrst")))
	{
		client_print_color(id, print_team_red, "^4#1FUP: ^3Ця команда доступна тільки VIP гравцям.");
		return PLUGIN_HANDLED;
	}
	
	g_bHideDamage[id] = !g_bHideDamage[id];
	
	if (g_bHideDamage[id])
		client_print_color(id, print_team_blue, "^4#1FUP: ^3Відображення урону ^4вимкнено^3.");
	else
		client_print_color(id, print_team_blue, "^4#1FUP: ^3Відображення урону ^4увімкнено^3.");
...
@CBasePlayer_TakeDamage_Post(id, inflictor, attacker, Float:damage, bitsDamageType)
{
	...
	{
		// Skip if player toggled damage display off
		if (!g_bHideDamage[attacker])
		{
			new color[3];
