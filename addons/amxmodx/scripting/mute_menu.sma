...
stock generate_dictionary() {
	...
	if(!file_exists(cfg_file)) {
		...
		write_file(cfg_file, "MM_MENU_TITLE = \r#1FUP:\w Mute Меню");
		write_file(cfg_file, "MM_MENU_ITEM_ALL = Замутить всех");
		write_file(cfg_file, "MM_MENU_LABEL_GAGGED = \r[\в муте\r]");
		write_file(cfg_file, "MM_ALERT_OPTION_SET = замутил");
		write_file(cfg_file, "MM_ALERT_OPTION_UNSET = снял мут с");
		write_file(cfg_file, "MM_ALERT_CHAT = Ты %L ^^3%n");
		write_file(cfg_file, "MM_ALERT_CHAT_ALL_SET = Ты замутил ^^3всех");
		write_file(cfg_file, "MM_ALERT_CHAT_ALL_UNSET = Ты снял мут ^^3со всех");
		write_file(cfg_file, "^n[en]");
		write_file(cfg_file, "MM_MENU_TITLE = \r#1FUP:\w Menu");
